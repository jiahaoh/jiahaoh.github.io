...
let setPlotlyTheme = (theme) => {
  document.querySelectorAll(".js-plotly-plot").forEach((elem) => {
    ...
    // Theme data loaded from plotly-themes.js (only on pages with Plotly charts)
    const themeLayout = theme === "dark" ? window.plotlyDarkLayout : window.plotlyLightLayout;

    if (themeLayout) {
      if (jsonData.layout) {
        if (jsonData.layout.template) {
          jsonData.layout.template = { ...themeLayout, ...jsonData.layout.template };
        } else {
          jsonData.layout.template = themeLayout;
        }
      } else {
        jsonData.layout = { template: themeLayout };
      }
